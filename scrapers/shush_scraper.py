...
class Shush_Scraper(scraper.Scraper):
    ...
    def _get_episode_url(self, show_url, season, episode, ep_title):
        episode_pattern = '<div class="list2"><a\s+href="([^"]+)"[^<]+Season\s+%s\s+Episode:\s+%s' % (season, episode)
        title_pattern= '<div class="list2"><a\s+href="([^"]+)"[^<]+Season\s+1\s+Episode:\s+\d+\s+-\s+([^<]+)'
        url = super(Shush_Scraper, self)._default_get_episode_url(show_url, season, episode, ep_title, episode_pattern, title_pattern)
